...
app.get("/api/get-rewards/:username", function(req, res){
  new sql.ConnectionPool(config.config_api).connect().then(pool => {
    ...
    .input("username",req.params.username)
    .query("SELECT * \
            FROM ( SELECT timestamp, permlink, TRY_CONVERT(float,REPLACE(reward,'VESTS','')) as reward, -1 as sbd_payout, -1 as steem_payout, -1 as vests_payout, type='paid_curation', DATEDIFF(day, GETDATE(), timestamp) as diff FROM VOCurationRewards WHERE curator='stoodkev' AND timestamp >= DATEADD(day,-14, GETUTCDATE()) AND timestamp < DATEADD(day,-7, GETUTCDATE()) \
              UNION ALL \
              SELECT timestamp, permlink, -1 as reward, sbd_payout, steem_payout, vesting_payout, type='paid_author', DATEDIFF(day, GETDATE(), timestamp) as diff FROM VOAuthorRewards WHERE author='stoodkev' AND timestamp >= DATEADD(day,-14, GETUTCDATE()) AND timestamp < DATEADD(day,-7, GETUTCDATE()) \
              UNION ALL \
              ...
              UNION ALL \
              SELECT timestamp, permlink, TRY_CONVERT(float,REPLACE(reward,'VESTS','')) as reward, -1 as sbd_payout, -1 as steem_payout, -1 as vests_payout, type='pending_curation', DATEDIFF(day, GETDATE(), timestamp) as diff FROM VOCurationRewards WHERE curator='stoodkev' AND timestamp >= DATEADD(day,-7, GETUTCDATE()) \
              UNION ALL \
              SELECT timestamp, permlink, -1 as reward, sbd_payout, steem_payout, vesting_payout, type='pending_author', DATEDIFF(day, GETDATE(), timestamp) as diff FROM VOAuthorRewards WHERE author='stoodkev' AND timestamp >= DATEADD(day,-7, GETUTCDATE()) \
              UNION ALL \
              SELECT timestamp, permlink, TRY_CONVERT(float,REPLACE(reward,'VESTS','')) as reward, -1 as sbd_payout, -1 as steem_payout, -1 as vests_payout, type='pending_benefactor', DATEDIFF(day, GETDATE(), timestamp) as diff FROM VOCommentBenefactorRewards WHERE benefactor='stoodkev' AND timestamp >= DATEADD(day,-7, GETUTCDATE()) \
            ) as rewards \
