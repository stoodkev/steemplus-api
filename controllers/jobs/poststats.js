...
exports.getPostStats = async function() {
  ...
  const title =
    ...
    new Date(Date.now())
      ...
      .replace(",", "");
  let body =
    "<div>https://cdn.steemitimages.com/DQmdUeahavHxrpFve3xXjTLNxkTuNeNiHxKwhBjciANZwNk/image.png <br><br>\
  ...
  <h3>Total</h3><br>\
  <p>A total of <strong>" +
    utils.numberWithCommas(stats.total_points) +
    "</strong> SPP has been distributed to <strong>" +
    utils.numberWithCommas(stats.points_per_user.length) +
    "</strong> users.</p>\
  ...
  for (let i = 0; i < 10; i++) {
    body +=
      ...
      (i + 1) +
      "</th><th>" +
      stats.points_per_user[i].name +
      "</th><th>" +
      utils.numberWithCommas(stats.points_per_user[i].points) +
      "</th><th>" +
      (i + 11) +
      "</th><th>" +
      stats.points_per_user[10 + i].name +
      "</th><th>" +
      utils.numberWithCommas(stats.points_per_user[10 + i].points) +
      "</th></tr>";
  }
  body +=
    "</table>\
  ...
  let data = stats.points_per_transaction.map(function(a) {
    return {x: a.type, value: a.points};
  });
  let chd = [];
  let chdl = [];
  let chl = [];
  ...
  let url = `https://image-charts.com/chart?cht=pd&chs=600x600&chd=a:${chd.join(
    ","
  )}&chdl=${chdl.join("|")}&chl=${chl.join(
    "|"
  )}&chco=380474|7171C6|3300FF|6666FF|836FFF|0276FD|0198E1|00B2EE|87CEFA|C6E2FF|BFEFFF`;

  const daily = await getDailyUsers();
  body += "<br><img src='" + url + "'/>";
  body +=
    "<br><h3>How to earn SPP?</h3><br>\
  ...
  <p>SteemPlus is a Chrome, Opera and Firefox extension used by " +
    daily.total +
    " users daily.<br>\
  ...
  const permlink = `spp-stats-${dateNow.getUTCFullYear()}-${dateNow.getUTCMonth() +
    1}-${dateNow.getUTCDate()}`;
...
function getDailyUsers() {
  ...
  const firefoxExtensionWebstoreURL =
    "https://addons.mozilla.org/en-US/firefox/addon/steem-plus/?src=search";
  return new Promise(function(fulfill, reject) {
    getJSON(
      "http://www.whateverorigin.org/get?url=" +
        encodeURIComponent(chromeExtensionWebstoreURL),
      function(e, response) {
        const users = response.contents.match(
          /<Attribute name=\"user_count\">([\d]*?)<\/Attribute>/
        )[1];
        getJSON(
          "http://www.whateverorigin.org/get?url=" +
            encodeURIComponent(firefoxExtensionWebstoreURL),
          function(e, firefox) {
            let users_firefox = firefox.contents.match(
              /<dd class=\"MetadataCard-content\">([/\d]*?)<\/dd>/
            );
            if (users_firefox == undefined) users_firefox = 0;
            else users_firefox = users_firefox[1];
            console.log({
              chrome: users,
              firefox: users_firefox,
              total: parseInt(users) + parseInt(users_firefox)
            });
            fulfill({
              chrome: users,
              firefox: users_firefox,
